...
my $frequencies = "frequencies_icelandic.txt";
my $ciphertext = "cipher_icelandic.txt";
...
while(<CIPHER>){
	...
	my @line = split(/ /, $_);#didn't like autosplitting the chars, so i padded spaces between in the ciphertext
	for ($i = 0; $i <= $#line; $i++){
		#print ("$index{$line[$i]} - $index{$map[10][$i % 10]} = " . (($index{$line[$i]} - $index{$map[10][$i % 10]}) % ($#revdex + 1)) . "\n");
		print ( $revdex[(($index{$line[$i]} - $index{$map[10][$i % 10]}) % ($#revdex + 1))] );
	}
}
close (CIPHER);
